import { coachMemoryLocalSchema, coachProfileLocalSchema, workoutAnalysisResultSchema } from "@/lib/analysis-zod";
import {
...
5) Antworte ausschließlich als gültiges JSON gemäß Schema.`;
...
export async function POST(request: Request) {
  ...
  const { result, answers, coachProfile, coachMemory } = parsed.data;
  const answersText =
    answers.length === 0
